   ...
   /* @Autowired//注意这个方法是注入的*/
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                .antMatchers("/historical_Marketing_Scheme").permitAll()
                .antMatchers("/merchantInformation/getMerchantDefineInformation").permitAll()
                .antMatchers("/sendEmail").permitAll()
                .antMatchers("/checkUserNameAndEmail").permitAll()
                .antMatchers("/updateSysUserPassword").permitAll()
                .antMatchers("/sendEmailMsg").permitAll()
                .antMatchers("/authentication/*").permitAll()
                .antMatchers("/test/*").permitAll()
                       /*..antMatchers("/SMSExecute/*").permitAll()*/
                .antMatchers("/baseLabel/get163LabelCity").permitAll()
                /*.antMatchers("/baseLabel/addLabels").permitAll()*/
